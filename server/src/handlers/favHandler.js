...
//* Handler que postea el producto fav en la DB
const postFavHandler = async (req, res) => {
  const { id_user, id_product } = req.params;
  try {
    await postFav(id_user, id_product);
    res.status(200).json("Producto añadido con éxito");
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
};
...
const deleteFavHandler = async (req, res) => {
  const { id_user, id_product } = req.params;
  try {
    await deleteFav(id_user, id_product);
